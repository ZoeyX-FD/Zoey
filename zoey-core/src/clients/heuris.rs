...
impl HeurisClient {
    ...
    pub async fn generate_image(&self, image_prompt: String) -> Result<Vec<u8>, anyhow::Error> {
        ...
        let request = client
            .request(
                reqwest::Method::POST,
                "https://sequencer.heurist.xyz/submit_job",
            )
            .headers(headers)
            .json(&body);
        ...
        if image_url.is_empty() {
            return Err(anyhow::anyhow!("Received empty image URL from Heuris API"));
        }

        if !image_url.starts_with("http") {
            return Err(anyhow::anyhow!("Invalid image URL received: {}", image_url));
        }
