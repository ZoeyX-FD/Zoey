...
use trader_solana::gmgn::client::GMGNClient;
use trader_solana::swap::SwapTool;
...
impl<E: EmbeddingModel + 'static> DirectClient<E> {
    ...
    pub async fn start(&self) {
        loop {
            ...
            if let Ok(response) = agent.prompt(Text::from(self.create_trends_prompt(&token_trending, &recent_trades).to_string())).await {
                ...
                if let Ok(recommendation) = serde_json::from_str::<serde_json::Value>(&response) {
                    ...
                    if let Ok(recommendation) = serde_json::from_str::<serde_json::Value>(&response) {
                        if let Some(tool) = recommendation["tool"].as_str() {
                            if !tool.is_empty() {
                                let action = agent.prompt(Text::from(tool)).await;
                                match action {
                                    Ok(action_str) => {
            ...
            if let Ok(response) = agent.prompt(Text::from(self.create_holdings_prompt(&holdings, &recent_trades).to_string())).await {
